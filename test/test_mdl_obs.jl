...
adc_vpp = 1. # in Volts
adc_nb_bits = 16
T_adc = adc_noise_temperature(adc_vpp, adc_nb_bits, freq_chan; instru_imp=50.)
T_LNA = 100. # in K
...
offset_angles = SphereCoord(-20., 0.) # (az,el) in degrees
...
T_gnd = ground_model(collect(0.:1.:360.), collect(0.:1.:180.), 30.)#FIXME:check value
...
#=
alpha_sky_grid = T_total_bkg.alpha_grid
beta_sky_grid = T_total_bkg.beta_grid
smap = T_total_bkg.spheremap
fig, axs = plt.subplots(subplot_kw=Dict("projection"=>"polar"))
...
img = axs.imshow(Array(F_src.flux), extent=plot_extent, aspect="auto", 
                 cmap="gist_earth", interpolation="nearest", origin="lower")
...
freq_bins = Array(dims(F_src.flux, :freqs))
fig, axs = plt.subplots()
axs.plot(time_bins, Array(F_src.flux[freqs=Near(cent_freq)]))
axs.set_xlabel("Time [UTC]")
...
## Satellite Antenna
# size of antenna in m
sat_ant_diameter = 3. #FIXME:check value
...
sat_gain_max = 10^(44/10) #FIXME:check value in dBi
...
sat_gain_pat = antenna_mdl_ITU_SA_509_3(sat_gain_max, half_beamwidth, alphas, betas)
sat_gain_pat[:,:gains] ./= maximum(sat_gain_pat[:,:gains]) # normalize and put in EIRP

# satellite antenna physical temperature
sat_T_phy = 0. # in K

# create satellite antenna
#FIXME: sat_ant = Antenna(sat_ant_diameter, sat_gain_pat, sat_eta_app, sat_eta_rad, freq_band,
#                   sat_T_phy)

alphas = [0., 180., 359.]
betas = 0.:1.:180.
sat_gain_pat = antenna_mdl_cst(1., alphas, betas)
sat_ant = Antenna(ant_diameter, sat_gain_pat, eta_app, eta_rad, freq_band, T_phy)
...
# sat_responsetype2 = Lowpass((125e6-freq_res) / (sat_bw/3.1))#FIXME:
...
max_EIRP_density = 10^(-11.9/10) * 1e-6 # in W/Hz #-15. + 10. * log10(300.) #FIXME:check value
...
sats_list = form_satellites_list(file_traj_sats_path, sat_instrument, max_EIRP_density,
                                 start_sat_window, stop_sat_window; 
                                 filt_funcs=(filt_name, filt_el), time_tag=:timestamp,
                                 sat_id_tag=:sat, elevation_tag=:elevations,
                                 azimuth_tag=:azimuths, range_tag=:ranges_westford)


## Constellation of Satellites
# satellite link budget estimator
lnk_bdgt(args...; kwds...) = classic_gain_link_budget(args...; simple_approx=true, 
                                                      beam_avoid_angle=0.,
                                                      turn_off=false, kwds...)
...
# create list of satellites
sats_list2 = form_satellites_list(file_traj_sats_path, sat_instrument2, max_EIRP_density,
                                 start_sat_window, stop_sat_window; 
                                 filt_funcs=(filt_name, filt_el), time_tag=:timestamp,
                                 sat_id_tag=:sat, elevation_tag=:elevations,
                                 azimuth_tag=:azimuths, range_tag=:ranges_westford)
